...
# 馬のトップページから、改行などを削除。     
  ...
  #  production = Date.strptime(birth,'%Y年%m月%d日') # 型を変換  
...
# csv書き込み umaname がカラの場合は書き込みしない。
        ...
        csv << [strurl,umaname,sex,birthday,all,win,second,third,other,money,grace,gracewin,production,finped] #csv書き込み finpedの配列をダブルクオートと[] を削除して書き込みできない
        # csv
